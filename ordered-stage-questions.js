(()=>{
  ...
  function spinToFirst(items,onDone){const reel=document.getElementById('stageReel'),all=allQuestions(),target=items[0],targetIndex=all.indexOf(target);renderStageQuestions();const metrics=reelMetrics(reel);wheelSound('start');const startIndex=all.length*10,finalIndex=startIndex+all.length*4+targetIndex,start=performance.now(),duration=4200;let last=-1;function frame(now){const p=Math.min(1,(now-start)/duration),ease=1-Math.pow(1-p,4),current=startIndex+(finalIndex-startIndex)*ease;reel.style.transform=`translateY(${-(current*metrics.step)+metrics.offset}px)`;const tick=Math.floor(current);if(tick!==last){wheelSound();last=tick}if(p<1)requestAnimationFrame(frame);else{wheelSound('win');onDone(target)}}requestAnimationFrame(frame)}
