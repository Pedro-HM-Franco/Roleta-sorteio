(()=>{
  ...
  const refresh=()=>{const s=data();if(!s?.roletas)return;axis.innerHTML=Object.keys(s.roletas.questions.groups).map(x=>`<option>${x}</option>`).join('');axis.style.display=kind==='question'?'block':'none';pending=null;draw.disabled=false;draw.textContent=kind==='question'?'Sortear pergunta':'Sortear';title.textContent=kind==='candidate'?'Candidatos':kind==='axis'?'Eixos temáticos':'Perguntas';out.textContent='Pronto para sortear';renderVerticalWheel('stageReel',list())};
  const sort=()=>{const items=list();if(!items.length){out.textContent='Nenhuma opção disponível';return}if(kind==='question'&&pending){addDraw('question',`Perguntas / ${axis.value}`,pending,axis.value);out.textContent=pending;pending=null;draw.textContent='Sortear pergunta';refresh();return}draw.disabled=true;spinVertical('stageReel',items,item=>{out.textContent=item;if(kind==='candidate')addDraw('candidate','Candidatos',item);if(kind==='axis')addDraw('axis','Eixos temáticos',item);if(kind==='question'){pending=item;draw.textContent='Confirmar pergunta'}draw.disabled=false})};
  open.onclick=e=>{e.preventDefault();stage.classList.remove('hidden');document.documentElement.classList.add('telão-open');refresh()};
  if(close)close.onclick=e=>{e.preventDefault();stage.classList.add('hidden');document.documentElement.classList.remove('telão-open')};
